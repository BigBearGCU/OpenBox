    ...
    def run(self):
        ...
        while self.go:
		    ...
		    #do we need this? some further test will be needed
                ...
                img=image
                #img=opencv.adaptors.Ipl2PIL(image)
                #frame_size = cv.cvGetSize (image)
                #camWidth=frame_size.width
                #camHeight=frame_size.height
                #camBuffer=image.imageData
                #print "Camera Buffer width ",camWidth," Height ",camHeight
	    ...
	    #match the res with the window size
            if (ret):
                ...
                camImage1 = pygame.image.frombuffer(destRGB.tostring(), (self.camWidth, self.camHeight), 'RGB')
                if gLinux:
                    if camBuffer:
                        del camBuffer
                if ret:
...
## Class XWebcam
# This class has responsibility for the creation and update of the webcam
...
class XWebcam(pygame.sprite.Sprite):
    ...
    def __init__(self):
	    ...
	    if (os.path.exists("/dev/video0")):
			...
			self.device.set(cv2.cv.CV_CAP_PROP_FRAME_WIDTH,camWidth)
			self.device.set(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT  ,camHeight)
            #windows and mac
