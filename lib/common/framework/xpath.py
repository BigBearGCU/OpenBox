...
class FilePath(ListPath):
    ...
    def loadLocations(self, file):
        self.places = xutils.load_points(file)


class LinePath(ListPath):
    """Starts here, goes there, and stops."""

    def __init__(self, startLocation=(20, 20), endLocation=(320, 400),
                    duration=None, steps=200):
        ListPath.__init__(self, duration=duration)            
        
        #places = xutils.load_points(fileName)        
        #Get point locations
        x0 = startLocation[0]
        y0 = startLocation[1]
        x1 = endLocation[0]
...
class LinePathEx(ListPath):
    """Starts here, goes there, then there, then there, and starts again."""

    print "In LinePathEx"
    
    def __init__(self, duration=6, steps01=0,spacing01=0, steps02=0,spacing02 = 0, steps03=0,spacing03 = 0,steps04=0,spacing04 = 0, file = None, file02 = None, file03 = None, file04 = None, debug = False, bounce = True ):
        ListPath.__init__(self, duration=duration)
        
        self.pathIndex = 0 
        self.totalNoOfPaths = 4
        self.completePaths = 0
        
        self.places01, self.places01Key  = self.generate_complete_points(file,steps01,bounce)
        self.places02, self.places02Key = self.generate_complete_points(file02,steps02,bounce)
        self.places03, self.places03Key  = self.generate_complete_points(file03,steps03,bounce)
        self.places04, self.places04Key = self.generate_complete_points(file04,steps04,bounce)
        #Hahahaaa, oh my.... Sorry!
        #self.places04 = xutils.load_points("path_04.txt")
        #self.places04Key = [(164, 348),(475, 348)] 
        
        self.listOfDotStep = [steps01,steps02,steps03,steps04]
        self.listOfDotSpace = [spacing01, spacing02, spacing03,spacing04]
        
        self.listOfPlaces = []        
        self.listOfPlaces = [self.places01,self.places02,self.places03,self.places04] 
        
        self.listOfPlacesKey = [self.places01Key,self.places02Key,self.places03Key,self.places04Key]
         
        self.places = self.listOfPlaces[0]
        self.set_position(self.places[0])
        self.set_timePerPlace(duration)

        
    def get_key_points(self, index):
        return self.listOfPlacesKey[index]
       
    def get_main_points(self): #Main pointsa are dots along the line
        return  self.places   
    
    def update_path(self,index):
        self.places = self.listOfPlaces[index]   
        
    def generate_complete_points(self, file, steps, bounce):  
        #Try loading point file
        try:            
            placesIn = xutils.load_points(file)
        except:
            print "Eroor, in LinePathEx, trying to load point list"
            return -1
        
        #Create increment valus for each line segment....
        incremntList = [] 
        segPerLine = []
        noOfSegments = len(placesIn)-1   
                    
        i = 0
        i1 = 1
        #For the number of points in the file - 1        
        for r in range(noOfSegments):
            if i1 == (noOfSegments+1):
                i1 = (noOfSegments) 
            
            #Get lengh of each "chunk" of line..            
            dx = placesIn[i1][0] - placesIn[i][0]
            dy = placesIn[i1][1] - placesIn[i][1]            
            #Return the Euclidean norm, sqrt(x*x + y*y)            
            lineLengh = math.hypot(dx, dy)            
          
            #Get the number of required segments per line "chunk"
            tempSegNo = (lineLengh/steps)
            segPerLine.append(int(tempSegNo))
            
            #Calcualte increment to move badge along line....                    
            x = (placesIn[i1][0] - placesIn[i][0]) / tempSegNo                
            y = (placesIn[i1][1] - placesIn[i][1]) / tempSegNo
            incremntList.append((x, y))  

            #Update index
            i +=1 
            i1 +=1
           
        #Get points to move through to interpolate along the entire line... 
        places = []
        stepsToTake = segPerLine[0]        
        xAdd = yAdd = 0
        i=0
        stepsToTake = segPerLine[i]
        startX, startY = placesIn[0]
        x,y = startX, startY

        #For the number of line segments we want...    
        for r in range(noOfSegments):
            stepsToTake = segPerLine[i]
            #print "stepsToTake", stepsToTake
            #While there are still steps left between...
            for s in range(stepsToTake):
                #add on the required incremnt
                xAdd += incremntList[i][0]
                yAdd += incremntList[i][1]
                x,y = (startX + xAdd), (startY + yAdd)
                places.append((x,y))
            i+=1 
...
class SquarePath(ListPath):
    ...
    def __init__(self, startLocation=(120, 0), startDirection=0,
                    duration=None, size=200, steps=800, clockwise=1):
        ListPath.__init__(self, duration=duration)
        stepsPerSide = int(steps / 3)
...
class CirclePath(ListPath):
    ...
    def __init__(self, startLocation=(0, 0), startDirection=0,
                    duration=None, size=200, steps=360, clockwise=1):
        ListPath.__init__(self, duration=duration)
        x, y = startLocation
...
class VelocityPathBounded(VelocityPath):
    ...
    def __init__(self, startLocation=(100, 100), vx=0, vy=0, duration=None,
                    xMin=20, xMax= WINWIDTH-20, yMin=20, yMax= WINHEIGHT-20,
                    endPathAtBoundary=1):
        VelocityPath.__init__(self, startLocation, vx, vy, duration=duration)
        self.xMin = xMin
        self.xMax = xMax
...
class RandomAccelerationPath(VelocityPathTime):
    ...
    Should be much smoother than BrownianPath since acceleration is limited.
